...
public class SubscriberCareServiceTest {
    ...
    @Before
    public void setUp() {
        subscriberCareService = new SubscriberCareService(allSubscriberCareDocs, reportingService);
    }
    ...
    @Test
    public void shouldReportSubscriptionCareDocRequest() {
        SubscriberCareRequest subscriberCareRequest = new SubscriberCareRequest(
                "1234567890", SubscriberCareReasons.HELP.toString(), Channel.CONTACT_CENTER.toString(), DateTime.now());

        subscriberCareService.create(subscriberCareRequest);

        ArgumentCaptor<SubscriberCareReportRequest> subscriberCareDocReportRequestCaptor = ArgumentCaptor.forClass(SubscriberCareReportRequest.class);
        verify(reportingService).reportCareRequest(subscriberCareDocReportRequestCaptor.capture());

        SubscriberCareReportRequest actualReportRequest = subscriberCareDocReportRequestCaptor.getValue();
        assertEquals(subscriberCareRequest.getMsisdn(), actualReportRequest.getMsisdn());
        assertEquals(subscriberCareRequest.getReason(), actualReportRequest.getReason());
        assertEquals(subscriberCareRequest.getChannel(), actualReportRequest.getChannel());
        assertEquals(subscriberCareRequest.getCreatedAt(), actualReportRequest.getCreatedAt());
    }
