...
public class SecondMainSubSlotMessagesSenderJobTest {
    ...
    public void setUp() {
        initMocks(this);
        when(obdProperties.getMainSlotCronJobExpressionFor(SubSlot.TWO)).thenReturn(cronJobExpression);
    ...
    public void shouldScheduleCronJobsAtConstruction() {
        ...
        assertEquals(SubSlot.TWO.name(), motechEvent.getParameters().get(MotechSchedulerService.JOB_ID_KEY));
    }
    ...
    public void shouldInvokeCampaignMessageServiceToSendSecondMainSubSlotMessagesWithRetryAndFulfillTheRetryIfSuccessful() {
        ...
        when(obdProperties.getMainSlotStartTimeLimitFor(subSlot)).thenReturn(DateTime.now().withHourOfDay(16).withMinuteOfHour(00));
        when(obdProperties.getMainSlotEndTimeLimitFor(subSlot)).thenReturn(DateTime.now().withHourOfDay(17).withMinuteOfHour(00));
