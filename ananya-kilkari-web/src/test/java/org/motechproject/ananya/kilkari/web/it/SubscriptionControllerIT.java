...
public class SubscriptionControllerIT extends SpringIntegrationTest {
    ...
    public void shouldRetrieveSubscriptionDetailsFromDatabaseForCC() throws Exception {
        ...
        markForDeletion(inboxMessage2);
        ReportingService mockedReportingService = mock(ReportingService.class);
        final SubscriberResponse reportResponse1 = new SubscriberResponse(subscription1.getSubscriptionId(), "beneficiaryName", 25, now, now, now, new LocationResponse("d", "b", "p"), now, now.minusDays(3), null);
        final SubscriberResponse reportResponse2 = new SubscriberResponse(subscription2.getSubscriptionId(), "beneficiaryName1", 26, now, now, now, new LocationResponse("d1", "b1", "p1"), now, now.minusDays(8), null);
